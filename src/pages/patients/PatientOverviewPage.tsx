  ...
  useEffect(() => {
    async function loadOverview() {
      try {
        ...
        // 2. Última Bioimpedância (Pega 2 para comparar)
        ...
        // 4. Próximo Agendamento (Placeholder - se tiver tabela appointments, conecte aqui)
        // const { data: appt } = ...
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="flex justify-between items-start mb-4">
                <div className="p-3 bg-pink-100 dark:bg-pink-900/30 rounded-full text-pink-600">
                    <Activity size={24} />
                </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            {bioCurrent ? (
                <>
                    ...
                    <p className="text-sm text-gray-500 mb-4">Última pesagem em {new Date(bioCurrent.data_avaliacao).toLocaleDateString('pt-BR')}</p>
                ...
                <div className="flex flex-col items-center justify-center h-32 text-gray-400">
                    <span className="text-sm">Sem dados corporais</span>
                </div>
            )}
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div className="space-y-4">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500">
                            {data.lastTreatment?.proxima_sessao 
                                ? new Date(data.lastTreatment.proxima_sessao).toLocaleDateString('pt-BR') 
                                : "Não agendado"}
                        </p>
                    </div>
                </div>
                ...
                <div className="flex items-start gap-3">
                    <div className="mt-1"><FileText size={16} className="text-gray-400"/></div>
                    <div>
                        <p className="text-sm font-bold text-gray-700 dark:text-gray-200">Pendências</p>
                        <p className="text-xs text-gray-500">Anamnese assinada? {data.lastTreatment ? 'Sim' : 'Verificar'}</p>
                    </div>
                </div>
            </div>
            ...
            <div className="mt-8 grid grid-cols-2 gap-3">
                {/* CORREÇÃO: Removemos o "../" */}
                <Button onClick={() => navigate("evolution")} variant="outline" className="text-xs">
                    + Evolução
                </Button>
                
                {/* CORREÇÃO: Caminho absoluto para sair do painel do paciente e ir para agenda geral */}
                <Button onClick={() => navigate("/appointments/new")} className="bg-purple-600 hover:bg-purple-700 text-white text-xs">
                    Agendar
                </Button>
            </div>
        </div>
      ...
      <div className="bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 p-6 rounded-2xl">
          <h3 className="font-bold text-yellow-800 dark:text-yellow-200 mb-2 flex items-center gap-2">
              <Clock size={18}/> Lembretes do Profissional
          </h3>
          <textarea 
            className="w-full bg-transparent border-none resize-none focus:ring-0 text-yellow-900 dark:text-yellow-100 text-sm placeholder-yellow-700/50"
            rows={3}
            placeholder="Digite aqui observações rápidas sobre este paciente (ex: 'Paciente prefere ser atendido à tarde', 'Sensível a dor')..."
          />
      </div>
