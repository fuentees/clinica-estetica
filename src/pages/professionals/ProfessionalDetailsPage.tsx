...
interface ProfessionalFormData {
  ...
  email: string;
  phone: string;
  role: string;              // <- continua usando "role" só no formulário
  formacao: string;          // Especialidade
  agenda_color: string;
  commission_rate: number;
  registration_number: string;
  avatar_url: string;
  signature_url?: string;    // assinatura digital
  working_days: string[];
  start_time: string;
  end_time: string;
  is_active: boolean;
...
export function ProfessionalDetailsPage() {
  ...
  const isMedicalStaff = ["profissional", "esteticista", "doutor", "medico", "biomedica"].includes(
    watchRole
  );
  const watchFirstName = watch("first_name");

  // --- CARREGAR PROFISSIONAL (modo edição) ---
  async function loadProfessional(profId: string) {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `
        id, first_name, last_name, name, email, phone, user_role, formacao,
        agenda_color, commission_rate, registration_number, avatar_url,
        signature_url, working_days, start_time, end_time, is_active
      `
      )
      .eq("id", profId)
      .single();
    ...
    if (data) {
      ...
      reset({
        ...
        email: data.email || "",
        phone: data.phone || "",
        role: data.user_role || "profissional",        // <- lê de user_role
  ...
  const uploadSignature = async (userId: string) => {
    if (!signatureFile) return null;
    const fileExt = signatureFile.name.split(".").pop();
    const fileName = `${userId}/${uuidv4()}-signature.${fileExt}`;
  ...
  const onSubmit = async (formData: ProfessionalFormData) => {
    ...
    try {
      const { first_name, last_name, role, ...rest } = formData;

      const dataToSave: any = {
        first_name,
        last_name,
        user_role: role,   // <- salva em user_role no banco
        ...rest,
      };

      // Criar novo se não tiver id
      if (!userId) {
      ...
      if (signatureFile && userId) {
        ...
        if (signatureUrl) {
          dataToSave.signature_url = signatureUrl;
        }
      }
  ...
  if (loading) {
    return (
      <div className="flex justify-center p-10">
        <Loader2 className="animate-spin mr-2 text-pink-600 w-8 h-8" />
      </div>
    );
  }

  const inputClass =
    "w-full p-2.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-pink-500 transition-all text-sm";
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 items-start">
                  ...
                  <div className="w-full h-full rounded-full border-4 border-pink-500/50 bg-gray-100 dark:bg-gray-700 flex items-center justify-center overflow-hidden">
                    {avatarPreview ? (
                      ...
                      <span className="text-4xl font-bold text-pink-600">
                        {watchFirstName?.[0]?.toUpperCase() || "U"}
                      </span>
                    )}
                  </div>
                  <label
                    htmlFor="avatar-upload"
                    className="absolute bottom-0 right-0 w-8 h-8 bg-pink-500 hover:bg-pink-600 text-white rounded-full flex items-center justify-center cursor-pointer shadow-md transition-colors"
                  >
                    <Camera size={16} />
                  </label>
                  <input
              ...
              <div className="md:col-span-3 grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    {...register("first_name", { required: "Nome obrigatório" })}
                ...
                <div>
                  ...
                  <input
                    {...register("last_name", { required: "Sobrenome obrigatório" })}
                    className={inputClass}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {isMedicalStaff && (
                <div className="md:col-span-2">
                  <label className={labelClass}>Número de Registro (CRM/CRBM/COFEN...)</label>
                  <div className="relative">
        ...
        <div className="space-y-6">
          ...
          {isMedicalStaff && (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
              <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-6 flex items-center gap-2">
                <Calendar size={16} /> Configurações de Agenda
              </h2>

              <label className={`${labelClass} flex items-center gap-1`}>
                <Clock size={12} /> Dias de Trabalho
              </label>
              ...
              <div className="grid grid-cols-7 gap-1 mb-6">
                {DAYS_OF_WEEK.map((day) => (
                  <label
                    key={day.value}
                    className={`relative block text-center border rounded-lg p-2 text-xs font-bold transition-colors cursor-pointer ${
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
              ...
              <div className="relative">
                <input
                  ...
                  className={`${inputClass} pr-8 font-bold text-right text-green-600`}
                  placeholder="0"
                  step="0.1"
                />
          ...
          {isMedicalStaff && (
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
              <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                <Camera size={16} /> Assinatura Digital
              </h2>

              <div className="space-y-3">
                <div className="w-full h-24 border border-dashed border-gray-300 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-900 flex items-center justify-center overflow-hidden">
                  {signaturePreview ? (
                    <img
                    ...
                    <span className="text-xs text-gray-400">Nenhuma assinatura enviada</span>
                  )}
                </div>

                <label
                  htmlFor="signature-upload"
                  className="inline-flex items-center justify-center px-3 py-2 rounded-lg bg-pink-600 hover:bg-pink-700 text-white text-xs font-semibold cursor-pointer"
                >
                ...
                <p className="text-[11px] text-gray-400">
                  Dica: use uma imagem em fundo branco (PNG ou JPG) com a sua assinatura escaneada.
                </p>
              </div>
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full h-12 bg-pink-600 hover:bg-pink-700 text-white font-bold rounded-xl shadow-lg shadow-pink-200 dark:shadow-none transition-all flex items-center justify-center gap-2"
          >
...
// também exporto como default, caso em algum lugar esteja usando import default
