...
export default function DashboardPage() {
  ...
  const profilePhoto = profile?.avatarUrl; // Assumindo que o campo no banco é avatarUrl
  ...
  async function fetchRealDashboardData() {
    ...
    try {
      ...
      const [patientsCount, newPatientsCount, apptsTodayCount, revenueData] = await Promise.all([
        supabase.from('patients').select('*', { count: 'exact', head: true }).eq('clinicId', profile.clinicId),
        supabase.from('patients').select('*', { count: 'exact', head: true }).eq('clinicId', profile.clinicId).gte('createdAt', firstDayMonth),
        supabase.from('appointments').select('*', { count: 'exact', head: true }).eq('clinicId', profile.clinicId).gte('startAt', startOfToday).lte('startAt', endOfToday).neq('status', 'cancelled'),
        supabase.from('payments').select('amount').eq('clinicId', profile.clinicId).gte('createdAt', firstDayMonth)
      ]);
      ...
      const { data: nextAppts } = await supabase
        ...
        .select(`
            ...
            professional:professionalId(fullName)
        `)
        .eq('clinicId', profile.clinicId)
        .gte('startAt', new Date().toISOString())
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const { count } = await supabase.from('appointments').select('*', { count: 'exact', head: true }).eq('clinicId', profile.clinicId).gte('startAt', startDay).lte('startAt', endDay);
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto min-h-screen bg-gray-50 dark:bg-gray-900 space-y-8 animate-in fade-in duration-700">
      ...
      {/* --- CABEÇALHO COM FOTO DO PROFISSIONAL --- */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCardPremium title="Ticket Médio" value={(stats.revenueMonth / (stats.totalPatients || 1)).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })} sub="Por paciente" icon={<Wallet size={24} />} color="purple" />
      </div>
              ...
              <div className="divide-y divide-gray-50 dark:divide-gray-700">
                  {nextAppointments.length === 0 ? (
                    ...
                    nextAppointments.map((appt) => (
                      <div key={appt.id} className="p-5 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => navigate(`/appointments/edit/${appt.id}`)}>
                          <div className="flex items-center gap-4">
                              ...
                              <div>
                                  ...
                                  <p className="text-xs text-gray-500 dark:text-gray-400">{appt.service?.name} • com {appt.professional?.fullName}</p>
                              </div>
                          </div>
...
// Mantido o componente StatCardPremium conforme a regra de não remover nada.
